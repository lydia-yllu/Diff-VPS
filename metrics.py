...
def _prepare_data(pred: np.ndarray, gt: np.ndarray) -> tuple:
    gt = gt > 128
    pred = pred / 255  # 归一化到[0,1]
    if pred.max() != pred.min():
        pred = (pred - pred.min()) / (pred.max() - pred.min())  # 最小-最大规范化
    return pred, gt
...
class Emeasure(object):
    ...
    def step(self, pred: np.ndarray, gt: np.ndarray):
        ...
        changeable_em = np.mean(changeable_ems, axis=0)
...
class Medical(object):
    ...
    def step(self, pred, gt, idx):
        ...
        self.Dice[idx, :] = metric.binary.dc(pred, gt)
        #print(time.time() - time_start)

    def get_results(self):

        column_Dic = np.mean(self.Dice, axis=0)
        #column_IoU = np.mean(self.threshold_IoU, axis=0)

        return dict(meanDice=column_Dic, maxDice=column_Dic)