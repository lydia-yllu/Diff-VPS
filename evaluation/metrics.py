...
# 阈值用于二值化，相当于设置一个界限，将像素值在两边的重新赋值为0和1
...
class Fmeasure(object):
    ...
    def cal_adaptive_fm(self, pred: np.ndarray, gt: np.ndarray) -> float:
        ...
        binary_predcition = pred >= adaptive_threshold  # 大于等于阈值的像素点为1，否则为0
        area_intersection = binary_predcition[gt].sum()  # pred和gt的交集
    ...
    def cal_pr(self, pred: np.ndarray, gt: np.ndarray) -> tuple:
        ...
        fg_hist, _ = np.histogram(pred[gt], bins=bins)  # 和gt的交集
        bg_hist, _ = np.histogram(pred[~gt], bins=bins)  # 取反（和gt相交的余集）
...
class Smeasure(object):
    ...
    def s_object(self, pred: np.ndarray, gt: np.ndarray) -> float:
        x = np.mean(pred[gt == 1])
        sigma_x = np.std(pred[gt == 1], ddof=1)  # 除以N-1
...
class Emeasure(object):
    ...
    def step(self, pred: np.ndarray, gt: np.ndarray, idx):
        ...
        # print('adaptive_em', adaptive_em)
        self.adaptive_ems.append(adaptive_em)
        # et = time.time()
        # totaltime = et -st
        # print('Emeasure:', totaltime)
...
class WeightedFmeasure(object):
    ...
    def step(self, pred: np.ndarray, gt: np.ndarray, idx):
        ...
        # et = time.time()
        # totaltime = et -st
        # print('WeightedFmeasure:', totaltime)
...
class Medical(object):
    ...
    def Fmeasure_calu(self, pred, gt, threshold):
        ...
        # 根据threshold将预测的概率二值化，一般默认threshold=0.5
        ...
        LabelAnd = (Label3 == 1) & (gt == True)
        NumAnd = np.sum(LabelAnd == 1)
        num_obj = np.sum(gt==True)
    ...
    def step(self, pred, gt, idx):
        ...
        for j, threshold in enumerate(self.Thresholds):
            # st = time.time()
            threshold_Dic[j], threshold_Rec[j] = self.Fmeasure_calu(pred, gt, threshold)
            # print("threshold_Dic[{}]:{}, threshold_Rec[{}]:{}".format(j, threshold_Dic[j], j, threshold_Rec[j]))
            # et = time.time()
            # totaltime = et -st
            # print('Medical:', totaltime)
        # print('threshold_Dic', threshold_Dic, idx)
        # print('threshold_Sensitivity', threshold_Rec)
        self.threshold_Dice[idx, :] = threshold_Dic  # idx * 256 -> idx
        self.threshold_Sensitivity[idx, :] = threshold_Rec

    def get_results(self):
        # print('self.threshold_Dice', self.threshold_Dice.shape, self.threshold_Dice)
        # print('self.threshold_Sensitivity', self.threshold_Sensitivity.shape, self.threshold_Sensitivity)
        column_Dic = np.mean(self.threshold_Dice, axis=0) # (length of a case, 256)/length (batch norm)
        column_Sen = np.mean(self.threshold_Sensitivity, axis=0)
        # print('column_Dic', column_Dic, 'column_Sen', column_Sen)
        return dict(maxDice=column_Dic[0], maxSen=column_Sen[0], meanSen=column_Sen[0])
